...
const RPGGrid: QuartzComponent = ({ options, ...props }: QuartzComponentProps & { options?: Options }) => {
  const folder = options?.folder ?? "rpgs"
  const title = options?.title ?? "RPGs"
  const limit = options?.maxDisplay ?? Infinity
  const className = options?.displayClass ?? "rpg-grid"
    ...
    <div className={`garden-section-container ${className}`}>
      ...
      <div className="rpg-grid-cards">
        {rpgPages.map((rpg) => {
          ...
          const rpgUrl = resolveRelative((props.fileData.slug ?? "") as FullSlug, rpg.slug!)
              ...
              <div className="card-content">
                ...
                {currentCampaign && (
                  <div className="rpg-card-status">
                    ...
                    <a
                      href={resolveRelative((props.fileData.slug ?? "") as FullSlug, currentCampaign.slug)}
...
export default ((userOpts?: Options) => {
  const RPGGridConstructor: QuartzComponent = (props) => {
    return <RPGGrid options={userOpts} {...props} />
  }
  return RPGGridConstructor
